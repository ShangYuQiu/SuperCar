...
//seguimiento del player
		for (int y = 0; y < game.getWidth(); y++) {
			...
			for (int x = 0; x < game.getVisibility(); x++) { //para ver la mapa completa solo hay que cambiar a game.getLenght;
				str.append(StringUtils.centre(game.getPositionToString(y, x + game.getPositionPc()), CELL_SIZE)).append(verticalDelimiter);
			}
